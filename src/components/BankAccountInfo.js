...
const CONNECT_BANK_REQUIRED_TEXT = 'Connecting your bank is required in order to participate with Cake Financials, this is how we pay you (and eventually ourselves).';
...
export class DashboardBankInfo extends Component {
    ...
    componentDidMount() {
        this.unsubscribeBankData = subscribeBankData(
            ({ bankData, loading, error }) => {
                this.setState({
                    error,
                    isLoading: loading,
    ...
    renderLoading = () => {
        return (
            <LoadingSpinner bsSize='large' text='Loading bank data...' />
        );
    }
