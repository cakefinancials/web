...
const StepsIcon = ({ stepClassName, ...props }) => {
  return <div {...props} className={`steps-header-icon ${stepClassName}`} />;
};
...
const CompletedCircle = ({ ...props }) => {
  ...
  return (
    <span {...props} className="fa-layers fa-fw lock-container">
      <i className="far fa-circle" style={CircleStyle} />
      <i className="fas fa-check" style={CircleCheckStyle} />
    </span>
  );
};
  ...
  render() {
    ...
    return (
      <div className="steps-header-container">
        <Steps current={-1}>
          {mapIndexed((val, idx) => {
            const [stepName, stepClassName, stepDescriptionText, stepHoverState, stepIconType] = val;
            ...
            return <Step key={idx} description={description} title={title} />;
