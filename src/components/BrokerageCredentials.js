    ...
    return (
      <Row>
        <Form horizontal onSubmit={this.handleSubmit}>
          <FormGroup controlId="brokerage" validationState={brokerageValidation ? null : 'error'}>
            <Col componentClass={ControlLabel} className={'cake-form-label'} xs={3}>
              Brokerage Website
            </Col>
            <Col xs={8}>
              <FormControl className={'cake-form-input'} onChange={this.handleChange} value={this.state.brokerage} />
              {brokerageValidation ? null : <HelpBlock>Your brokerage website cannot be blank</HelpBlock>}
            </Col>
            <Col className="checked-lock-icon" xs={1}>
              <Lock check />
            </Col>
          </FormGroup>
          <FormGroup controlId="username" validationState={usernameValidation ? null : 'error'}>
            <Col componentClass={ControlLabel} className={'cake-form-label'} xs={3}>
              Brokerage Username
            </Col>
            <Col xs={8}>
              <FormControl className={'cake-form-input'} onChange={this.handleChange} value={this.state.username} />
          ...
          <FormGroup controlId="password" validationState={passwordValidation ? null : 'error'}>
            <Col componentClass={ControlLabel} className={'cake-form-label'} xs={3}>
              Brokerage Password
            </Col>
            <Col xs={8}>
...
export class ObfuscatedBrokerageCredentials extends Component {
  ...
  renderLoading = () => {
    return <LoadingSpinner bsSize="large" text="Loading existing brokerage credentials..." />;
  };
  ...
  renderLoaded = () => {
    return (
      <Fragment>
        <span>
          Brokerage: <strong>{this.state.obfuscatedBrokerage || 'N/A'}</strong>
        </span>
        <br />
        <br />
  ...
  render = () => {
    return <div>{this.state.isLoading ? this.renderLoading() : this.renderLoaded()}</div>;
  };
}
