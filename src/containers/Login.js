  ...
  render() {
    return (
      <div className="Login auth-form-container center-text">
        ...
        <br />
        <small>
          <Link to="/forgot-password">Forgot your password? Click here to reset it.</Link>
        </small>
        {!R.path(['location', 'state', 'emailVerified'], this.props) ? null : (
          <div className="login-toast">{"You've successfully confirmed your account, please login to continue"}</div>
        )}
        {!R.path(['location', 'state', 'passwordReset'], this.props) ? null : (
          <div className="login-toast">{"You've successfully reset your password, please login to continue"}</div>
        )}
      </div>
    );
  }
